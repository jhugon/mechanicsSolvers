...
def difftotal(expr, diffby, diffmap):
  ...
  """
    ...
    derivmap = {
        Derivative(Function(v)(diffby), diffby): dv for v, dv in diffmap.items()
    }
...
def makeMomentaHamiltonian(lagrangian, coordinates, velocities, timeDerivMap):
  ...
  """
    ...
    momentaDefs = [
        Eq(mom, momInTermsOfVel)
        for mom, momInTermsOfVel in zip(momenta, momentaInTermsOfVel)
    ]
    ...
    for mom, vel, momInTermsOfVel in zip(momenta, velocities, momentaInTermsOfVel):
        ...
        if len(slnList) != 1:
            raise Exception(
                "Wrong number of solutions inverting for velocity in terms of momentum, {0} solutions".format(
                    len(slnList)
                )
            )
    ...
    hamiltonian = hamiltonian.subs(
        {
            vel: velInTermsOfMom
            for vel, velInTermsOfMom in zip(velocities, velocitiesInTermsOfMom)
        }
    )
...
class SymComputer(object):
    def __init__(self, lagrangian, coordinates, velocities, t=Symbol("t")):
        for coord in coordinates:
            if str(coord) == str(t):
                raise Exception(
                    "Coordinate '{0}' and time variable '{1}' have the same name".format(
                        str(coord), str(t)
                    )
                )
    ...
    def solveEulerLegrange(self):
        self.eomsL = eulerLegrangeEOM(
            self.lagrangian, self.coordinates, self.velocities, self.t, self.tdm
        )
        self.haveEOMsL = True

    def solveHamiltonian(self):
        self.momenta, self.momentaDefs, self.hamiltonian = makeMomentaHamiltonian(
            self.lagrangian, self.coordinates, self.velocities, self.tdm
        )
        self.eomsH = hamiltonianEOM(
            self.hamiltonian, self.coordinates, self.momenta, self.tdm
        )
        self.haveEOMsH = True
...
class MatrixLagrangian(object):
    ...
    def __init__(
        self, kineticMatrix, velocityColumn, noVelocityTerms, coordinates, velocities
    ):
        """
        ...
        if not isinstance(kineticMatrix, Matrix):
            raise Exception(
                "KineticMatrix {} is not of type Matrix".format(kineticMatrix)
            )
        if not isinstance(velocityColumn, Matrix):
            if not (velocityColumn is None):
                raise Exception(
                    "velocityColumn {} is not of type Matrix".format(velocityColumn)
                )
    ...
    def getLagrangian(self):
        ...
        return (
            self.velocitiesRow * self.kineticMatrix * self.velocitiesColumn / 2
            + velocityDependence
            + self.noVelocityTerms
        )
    ...
    def getInvertedKineticHamiltonian(self):
        ...
        momentaInTermsOfVel = (
            self.kineticMatrix * self.velocitiesColumn + self.velocityColumn
        )
        momentaDefs = [
            Eq(mom, momInTermsOfVel)
            for mom, momInTermsOfVel in zip(momenta, momentaInTermsOfVel)
        ...
        hamiltonian = (kineticMomentaColumn.T) * invKinetic * (
            kineticMomentaColumn
        ) / 2 - self.noVelocityTerms
